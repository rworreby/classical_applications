...
class SP1D(object):
    ...
    def addVelocityPerturb(self, lam, v0, sin=False):
        ...
        self.psi = self.psi * np.exp(phi / self.hbar_ * 1.0j)
        self.psi = self.psi / self.getNorm()
    ...
    def update(self, dt_, iters):
        for j in range(iters):
            ...
            psi_k *= np.exp(
                -1j * dt_ * self.T_scale * self.hbar * (self.kx ** 2) / (4 * self.mpart)
            )
...
class SP2D(object):
    ...
    def setThermalDistr(self, Sa, v_th, lam0=1.0, dn=0.0, v_bump=0.0):
        ...
        for i in range(len(Sa)):
            ...
            psiX_ = np.exp(phi / self.hbar_ * 1.0j)
    ...
    def addVelocityPerturb(self, lamx, lamy, v0, sin=False):
        ...
        psiX = np.exp(phiX / self.hbar_ * 1.0j)
        psiY = np.exp(phiY / self.hbar_ * 1.0j)
