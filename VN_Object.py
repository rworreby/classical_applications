...
class VN1D(object):
    def __init__(
        ...
        self.x = self.dx * (0.5 + np.arange(-self.N / 2, self.N / 2))

        self.sig_x = self.dx * f

        self.T_scale = (
            np.pi * 2.0 / np.sqrt(np.abs(C * Mtot / L))
        )  # time scaling factor
    ...
    def setThermalDistr(self, Sa, v_th, lam0=1.0, dn=0.0, v_bump=0.0):
        v = (
            self.hbar_
            * 4
            * np.pi
            * np.pi
            * np.array(Sa)
            / (lam0 * self.L * np.sqrt(self.C))
        )
        w_pre = np.exp(-((v / v_th) ** 2) + 0j) + dn * np.exp(
            -(((v - v_bump) / v_th) ** 2) + 0j
        )
        w_ = u.normalize(w_pre, 1)

        for i in range(len(Sa)):
            phi = self.x * self.hbar_ * np.pi * 2.0 * Sa[i] / self.L
            psi_ = np.exp(phi / self.hbar_ * 1.0j)
    ...
    def addVelocityPerturb(self, lam, v0, sin=False):
        ...
        for i in range(len(lam)):
            phi += (
                ...
                * lam[i]
                / (2 * np.pi)
                * np.cos(2 * np.pi * self.x / lam[i] + np.pi / 2.0 * sin)
                / len(lam)
            )
        psi_ = np.exp(phi / self.hbar_ * 1.0j)
    ...
    def addSpatialPerturb(self, lam, delta0, sin=False):
        ...
        for i in range(n_lam):
            psi_ += (
                delta0
                * np.sin(2.0 * np.pi * self.x / (lam[i] * self.L) + np.pi / 2.0 * sin)
                / n_lam
            )
        psi_ = np.sqrt(psi_)
        self.rho = self.rho * np.outer(np.conj(psi_), psi_)
...
class VN2D(object):
    ...
    def getRhoU(self):
        rho_u = sp.ifft(
            sp.fft(sp.ifft(sp.fft(self.rho, axis=3), axis=2), axis=1), axis=0
        )
    ...
    def setThermalDistr(self, Sa, v_th, lam0=1.0, dn=0.0, v_bump=0.0):
        ...
        x_ = self.dx * (0.5 + np.arange(-self.N / 2, self.N / 2))
        rhoX = np.zeros((N, N)) + 0j
        for i in range(len(Sa)):
            phi = x_ * self.hbar_ * np.pi * 2.0 * Sa[i] / self.L
            psi_ = np.exp(phi / self.hbar_ * 1.0j)
    ...
    def addVelocityPerturb(self, lamX, lamY, v0, sin=False):
        ...
        for i in range(len(lamX)):
            phiX += (
                ...
                * lamX[i]
                / (2 * np.pi)
                * np.cos(2 * np.pi * x_ / lamX[i] + np.pi / 2.0 * sin)
                / len(lamX)
            )
        for i in range(len(lamY)):
            phiY += (
                ...
                * lamY[i]
                / (2 * np.pi)
                * np.cos(2 * np.pi * x_ / lamY[i] + np.pi / 2.0 * sin)
                / len(lamY)
            )
        psiX_ = np.exp(phiX / self.hbar_ * 1.0j)
        rhoX = np.outer(np.conj(psiX_), psiX_)
        psiY_ = np.exp(phiY / self.hbar_ * 1.0j)
    ...
    def addSpatialPerturb(self, lamx, lamy, delta0, sin=False):
        ...
        for i in range(n_lamx):
            psiX_ += (
                delta0
                * np.sin(2.0 * np.pi * x_ / (lamx[i] * self.L) + np.pi / 2.0 * sin)
                / n_lamx
            )
        for i in range(n_lamy):
            psiY_ += (
                delta0
                * np.sin(2.0 * np.pi * x_ / (lamy[i] * self.L) + np.pi / 2.0 * sin)
                / n_lamy
            )
        psiX_ = np.sqrt(psiX_)
        rhoX = np.outer(np.conj(psiX_), psiX_)
    ...
    def update(self, dt_, iters):
        for j in range(iters):
            ...
            self.rho *= self.HK
            self.rho = sp.fft(
                sp.ifft(sp.fft(sp.ifft(self.rho, axis=3), axis=2), axis=1), axis=0
            )
